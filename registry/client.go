...
func RegisterService(r Registration) error { //然后客户端serviceUpdateUrl，服务注册信息向这个url更新信息， 需要提前处理下
	...
	//	return err
	//}
	http.Handle(serviceUpdateURL.Path, &serviceUpdateHanlder{}) //绑定handle进行处理
...
func (suh serviceUpdateHanlder) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost { //必须是post请求
...
func ShutdownService(url string) error {
	req, err := http.NewRequest(http.MethodDelete, ServicesURL,
		bytes.NewBuffer([]byte(url))) //应该使用注册的url
	if err != nil {
		return err
	}
	req.Header.Add("Content-Type", "text/plain") //请求头文件
...
type providers struct { //garding service 需要log service， logservice相对于grading就是一个provider，可能有多个rul
	services map[ServiceName][]string
	mutex    *sync.RWMutex
}

func (p *providers) Update(pat patch) { //接收到patch就要更新 注意用到是指针
	p.mutex.Lock()
